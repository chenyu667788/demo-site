...
const EVONET_CONFIG = {
    ...
    DROPIN_API: 'https://sandbox.evonetonline.com/dropin',
...
app.post('/api/subscription/create', async (req, res) => {
    try {
        ...
        try {
            ...
            linkUrl = apiResponse.linkUrl || apiResponse.paymentUrl;
        } catch (error) {
            console.error('API call error:', error);
        }
        
        subscription.sessionId = sessionId;
        subscription.linkUrl = linkUrl;
...
app.post('/api/subscription/:subscriptionId/bind-card', async (req, res) => {
    ...
    try {
        ...
        const linkUrl = apiResponse.linkUrl || apiResponse.paymentUrl;
...
app.listen(port, () => {
    console.log(`Drop-in Subscription Demo server running at http://localhost:${port}`);
    console.log(`Subscription plans page: http://localhost:${port}/index.html`);
    console.log(`\n订阅支付流程说明:`);
    console.log(`1. 首次订阅: 用户选择方案后使用Drop-in完成支付`);
    console.log(`2. Token保存: 支付时勾选"Save card details for next time"保存token`);
