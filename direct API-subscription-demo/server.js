...
app.post('/api/subscription/:subscriptionId/create-dropin-session', async (req, res) => {
    try {
        ...
        const requestData = {
            merchantOrderInfo: {
                merchantOrderID: subscription.orderId,
        ...
        subscription.sessionId = result.sessionID;
...
app.post('/api/direct-payment', async (req, res) => {
    ...
    try {
        ...
        const paymentRequestData = {
            merchantTransInfo: {
                merchantTransID: merchantTransID || `PAY-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
        ...
        if (saveCard && customerId) {
            paymentRequestData.paymentMethod.recurringProcessingModel = 'Subscription';
        }
        
        console.log('Direct Payment API request:', JSON.stringify(paymentRequestData, null, 2));
...
app.post('/api/subscription/:subscriptionId/direct-payment', async (req, res) => {
    ...
    try {
        ...
        const paymentRequestData = {
            ...
            returnURL: `http://localhost:${port}/success.html?subscriptionId=${subscription.id}`,
            webhook: `http://localhost:${port}/webhook/subscription`
        };
        
        console.log('Subscription Direct Payment API request:', JSON.stringify(paymentRequestData, null, 2));
...
app.listen(port, () => {
    console.log(`Direct API Subscription Demo server running at http://localhost:${port}`);
    console.log(`Subscription plans page: http://localhost:${port}/index.html`);
    console.log(`\n订阅支付流程说明:`);
    console.log(`1. 首次订阅: 用户选择方案后进入Direct API支付页面`);
    console.log(`2. Token保存: 支付成功后自动保存token用于后续扣费`);
